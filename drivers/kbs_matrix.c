...
#include "scicodes.h"
#include "smc.h"
#if CONFIG_EARLY_KEY_SEQUENCE_DETECTION
...
#if CONFIG_GPIO_KSO_DRIVE
static void kscan_ksodrive(const struct device *dev, int col, bool level);
#endif
...
#define MAX_SC2_TABLE_SIZE 135U
...
/**
 * @brief Check if numlock is currently enabled
 * @return true if numlock is on, false otherwise
 */
...
/**
 * @brief Check if scroll lock is currently enabled
...
/**
 * @brief Check if Fn key is currently pressed
 * @return true if Fn is pressed, false otherwise
 */
...
/**
 * @brief Check if Ctrl key is currently pressed
...
/**
 * @brief Check if Alt key is currently pressed
...
/**
 * @brief Check if Shift key is currently pressed
...
/**
 * @brief Toggle numlock state on key press
...
/**
 * @brief Toggle scroll lock state on key press
 * @param pressed true if key is pressed, false if released
 */
...
/**
 * @brief Initialize keyboard matrix scanning
 * @param callback Function to call when scan codes are generated
 * @param initial_set Pointer to initial scan code set
...
#if CONFIG_GPIO_KSO_DRIVE
	kscan_ksodrive_config(kscan_dev, kscan_ksodrive);
#endif
...
/**
 * @brief Write typematic rate and delay settings
 * @param data Typematic configuration byte
 */
...
/**
 * @brief Reset keyboard to default settings
 */
...
/**
 * @brief Check if a hotkey was detected
 * @return true if hotkey detected, false otherwise
 */
...
/**
 * @brief Update modifier key states based on key press/release
 * @param key_num Key number to check
 * @param pressed true if key is pressed, false if released
 */
...
/**
 * @brief Get print screen scan code based on modifier state
 * @param sc2 Output scan code structure
 * @param pressed true if key is pressed, false if released
 * @param ctrl_pressed true if Ctrl is held
 * @param alt_pressed true if Alt is held
 * @param shift_pressed true if Shift is held
 */
...
/**
 * @brief Get pause/break scan code
 * @param sc2 Output scan code structure
 * @param ctrl_pressed true if Ctrl is held
 */
...
/**
 * @brief Get numpad scan code for keyboards without dedicated numpad
 ...
 * @param key_num Key number that was pressed
 */
 ...
 * @param key_num Key number that was released
 */
...
/**
 * @brief Typematic timer callback for key repeat
...
#if CONFIG_EARLY_KEY_SEQUENCE_DETECTION
/**
 * @brief Check if a boot key sequence was detected
 * @param type Type of key sequence to check
 * @return true if sequence was detected, false otherwise
 */
...
/**
 * @brief Define a runtime hotkey sequence
 * @param modifiers Modifier key flags required
 * @param key Trigger key number
 * @param callback Function to call when sequence is detected
 * @return 0 on success, negative error code on failure
 */
...
/**
 * @brief Register a callback for a predefined key sequence
 * @param index Key sequence index
 * @param callback Function to call when sequence is detected
 * @return 0 on success, negative error code on failure
 */
 ...
 * @param last_key Key number to check
 * @return true if key is a modifier, false otherwise
 */
...
/**
 * @brief Detect firmware hotkey sequences
 * @param pressed true if key is pressed, false if released
...
/**
 * @brief Keyboard scan callback for key press/release events
 * @param dev Pointer to kscan device
...
#if CONFIG_GPIO_KSO_DRIVE
static uint32_t KSO_SCAN_PinTable[] = {
...
/**
 * @brief Set KSO line high
 * @param line KSO line number
 */
void KSO_Out_High(uint8_t line)
{
	if (line < MAX_MTX_KEY_COLS)
		gpio_write_pin(KSO_SCAN_PinTable[line], 1);
}

/**
 * @brief Set KSO line low
 * @param line KSO line number
 */
void KSO_Out_Low(uint8_t line)
{
	if (line < MAX_MTX_KEY_COLS)
		gpio_write_pin(KSO_SCAN_PinTable[line], 0);
}

/**
 * @brief Set all KSO lines high
 */
void KSO_All_High(void)
{
	uint8_t i;
	for (i = 0; i < MAX_MTX_KEY_COLS; i++)
		KSO_Out_High(i);
}

/**
 * @brief Set all KSO lines low
 */
void KSO_All_Low(void)
{
	uint8_t i;
	for (i = 0; i < MAX_MTX_KEY_COLS; i++)
		KSO_Out_Low(i);
...
/**
 * @brief KSO drive callback for keyboard scanning
 * @param dev Pointer to kscan device
 * @param col Column to drive (-2 for all high, -1 for all low)
 * @param level Output level for specific column
 */
static void kscan_ksodrive(const struct device *dev, int col, bool level)
{
	if (col == -2) {
...
/**
 * @brief Generate Windows slide key combination
 */
void kbs_generate_key_win_slide(void)
{
	make_key(KM_LWIN_KEY);
	k_busy_wait(1 * USEC_PER_MSEC);
	make_key(KM_WRFRSH_KEY);
	k_busy_wait(1 * USEC_PER_MSEC);
	break_key(KM_WRFRSH_KEY);
	k_busy_wait(1 * USEC_PER_MSEC);
	break_key(KM_LWIN_KEY);
}