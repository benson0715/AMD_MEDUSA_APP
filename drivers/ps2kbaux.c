...
/**
 * @brief Convert scan code set 1 to key number
 *
 * @param sc1 Scan code set 1 value
 * @param key_num Pointer to store the key number
 * @return 0 on success, -EINVAL if scan code not recognized
 */
...
/**
 * @brief Check if Ctrl+Alt+Shift combination is pressed
 *
 * @param data Scan code data byte
 * @return true if all three modifier keys are pressed, false otherwise
 */
...
/**
 * @brief Execute callback control operation with retries
 *
 * @param cmd Callback command (enable or disable)
 * @param dev PS2 device pointer
...
/**
 * @brief PS2 keyboard data callback handler
 *
 * @param dev PS2 device pointer
 * @param value Received scan code value
 */
...
/**
 * @brief PS2 mouse data callback handler
 *
 * @param dev PS2 device pointer
 * @param value Received mouse data value
 */
...
/**
 * @brief Initialize PS2 keyboard interface
 *
 * @param callback Application callback for keyboard data
...
int ps2_keyboard_init(const ps2_callback callback, uint8_t *initial_set)
{
	...
	if (!device_is_ready(keyboard_dev)) {
		LOG_ERR("PS2 keyboard device not ready");
		return -EIO;
	}
...
/**
 * @brief Write data to PS2 keyboard
 *
...
/**
 * @brief Disable PS2 keyboard callback
 */
...
/**
 * @brief Enable PS2 keyboard callback
 */
...
/**
 * @brief Initialize PS2 mouse interface
 *
 * @param callback Application callback for mouse data
 * @return 0 on success, negative error code on failure
 */
...
/**
 * @brief Disable PS2 mouse callback
 */
...
/**
 * @brief Enable PS2 mouse callback
 */
