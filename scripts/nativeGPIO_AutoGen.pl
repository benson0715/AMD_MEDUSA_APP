...
sub calcPinDefinition()
{
	...
	foreach my $key (sort { $PIN_DEF{$a}->{PIN_NAME} cmp $PIN_DEF{$b}->{PIN_NAME} } keys %PIN_DEF) {
		...
		if ($pinName =~ /GPIO(\d{3})/) {
			...
			if ($pinPwr =~ /VTRrel/i) {
				$u32Setting |= (0x03 << 2);
				# $strSetting .= "MCHP_GPIO_CTRL_PWRG_RSVD, ";
				$thisPin->{PIN_PWR_STR} = sprintf("MCHP_GPIO_CTRL_PWRG_RSVD");
			} elsif ($pinPwr =~ /VCC/i) {
				$u32Setting |= (0x01 << 2);
				# $strSetting .= "MCHP_GPIO_CTRL_PWRG_VCC_IO  , ";
				$thisPin->{PIN_PWR_STR} = sprintf("MCHP_GPIO_CTRL_PWRG_VCC_IO");
			} elsif ($pinPwr =~ /PWR_OFF/i) {
				$u32Setting |= (0x02 << 2);
				# $strSetting .= "MCHP_GPIO_CTRL_PWRG_OFF   , ";
				$thisPin->{PIN_PWR_STR} = sprintf("MCHP_GPIO_CTRL_PWRG_OFF");
			} elsif ($pinPwr =~ /VTR/ || $pinPwr =~ /^\s*$/) {
				$u32Setting |= (0x00 << 2);
				# $strSetting .= "MCHP_GPIO_CTRL_PWRG_VTR_IO, ";
				$thisPin->{PIN_PWR_STR} = sprintf("MCHP_GPIO_CTRL_PWRG_VTR_IO");
			} else {
			...
			$thisPin->{PIN_CTRL_MACRO} = sprintf("    {%-20s%-60s%-27s},", $thisPin->{NET_NAME}.", ", $thisPin->{VAL_STR}.$thisPin->{DIR_STR}.$thisPin->{INT_TYPE_STR}.$thisPin->{INTERNAL_PAD_STR}.", ",$thisPin->{PIN_PWR_STR});
			# # DEBUG OVERRIDE
			...
			$thisPin->{PIN_OPT_MACRO}  = sprintf("#define %-24s EC_GPIO_%03d", $thisPin->{srcNET_NAME}, $idx);
...
sub _genRuntimeSwitchMacros ()
{
	...
	foreach my $key (sort { $PIN_DEF{$a}->{PIN_NAME} cmp $PIN_DEF{$b}->{PIN_NAME} } keys %PIN_DEF) {
		...
		if ($thisPin->{$st} !~ /^\s*$/) {
		 ...
		 $buf .= sprintf("    {%-20s%-45s%-27s},    \\\n", $thisPin->{NET_NAME}.", ", $thisPin->{VAL_STR}.$thisPin->{DIR_STR}.$thisPin->{INT_TYPE_STR}.$thisPin->{INTERNAL_PAD_STR}.", ","MCHP_GPIO_CTRL_PWRG_OFF");
				} else {
		 $buf .= sprintf("    {%-20s%-45s%-27s},    \\\n", $thisPin->{NET_NAME}.", ", $thisPin->{VAL_STR}.$thisPin->{DIR_STR}.$thisPin->{INT_TYPE_STR}.$thisPin->{INTERNAL_PAD_STR}.", ","MCHP_GPIO_CTRL_PWRG_VTR_IO");
				}
			} else {
...
/* APP-owned gpios */\n";
	...
	# generate GPIO Ctrls
	$fileBuf .= "struct gpio_ec_config AMDMEC1723_CFG[] =  {";
...
{
    int ret = 0;
	ret = gpio_configure_array(AMDMEC1723_CFG, ARRAY_SIZE(AMDMEC1723_CFG));
    return ret;
}\n";
	...
	$fileBuf .= " ***********************************************************************/\n";
	$fileBuf .= "
struct gpio_ec_config _s_AMDMEC1723_cfg_rtG3[] = {
    GPIO_RUNTIME_SWITCH_G3
};
struct gpio_ec_config _s_AMDMEC1723_cfg_rtS5S4[] = {
    GPIO_RUNTIME_SWITCH_S5S4
};
struct gpio_ec_config _s_AMDMEC1723_cfg_rtS3[] = {
    GPIO_RUNTIME_SWITCH_S3
};
struct gpio_ec_config _s_AMDMEC1723_cfg_rtS0[] = {
...
void __autoGen_runtimeGpioSwitching (enum system_power_state pwr_state ) {
    switch (pwr_state) {
        case SYSTEM_G3_STATE:   
			gpio_configure_array(_s_AMDMEC1723_cfg_rtG3, ARRAY_SIZE(_s_AMDMEC1723_cfg_rtG3));   
			break;
        case SYSTEM_S4_STATE:
        case SYSTEM_S5_STATE:
			gpio_configure_array(_s_AMDMEC1723_cfg_rtS5S4, ARRAY_SIZE(_s_AMDMEC1723_cfg_rtS5S4)); 
			break;
        case SYSTEM_S3_STATE:   
			gpio_configure_array(_s_AMDMEC1723_cfg_rtS3, ARRAY_SIZE(_s_AMDMEC1723_cfg_rtS3));   
			break;
        case SYSTEM_S0_STATE:
			gpio_configure_array(_s_AMDMEC1723_cfg_rtS0, ARRAY_SIZE(_s_AMDMEC1723_cfg_rtS0));
			break;  
        default:                                                   
			break;
    }
