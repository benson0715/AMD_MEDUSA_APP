...
void IOExp_P3_PD_FORCE_FW_UPn(uint8_t pinSt)
{
	struct inputBuffer_info *info;
    info = NULL;
	//info = periph_get_gpio_info(ex_PD_FORCE_FW_UPn); /* RTK_TODO */
	if (info == NULL) {
...
void IOExp_P6_MEM_PMIC_PWR_GOOD(uint8_t pinSt)
{
	struct inputBuffer_info *info;
    info = NULL;
	//info = periph_get_gpio_info(ex_MEM_PMIC_PWR_GOOD); /* RTK_TODO */
	if (info == NULL) {
		LOG_ERR("Cannot find pin info");
		return;
	}
...
void IOExp_P11_UDC_EP_DBG_CARD_PRSNTn(uint8_t pinSt)
{
	struct inputBuffer_info *info;
	info = NULL;
    //info = periph_get_gpio_info(ex_UDC_EP_DBG_CARD_PRSNTn); /* RTK_TODO */
	if (info == NULL) {
		LOG_ERR("Cannot find pin info");
		return;
	}
...
void IOExp_P12_EC_UART_DOGL_PRSNTn(uint8_t pinSt)
{
	struct inputBuffer_info *info;
    info = NULL;
	//info = periph_get_gpio_info(ex_EC_UART_DOGL_PRSNTn); /* RTK_TODO */
	if (info == NULL) {
		LOG_ERR("Cannot find pin info");
		return;
	}
...
void periph_debouncer_LidClose(uint8_t pin_evt)
{
	...
	if ( pinVal && _s_InputBufferLidSt && !_s_InputBufferLidLastSend ) {
		...
		if(app_pseq_getPwrStatus() > SYSTEM_G3_STATE) {
			...
			//ioexp_set(ex_EP_TPAD_DISn, 1); //RTK_TODO
		}
		...
		if(app_pseq_getPwrStatus() > SYSTEM_G3_STATE) {
			//ioexp_set(ex_EP_TPAD_DISn, 0); /* Set TPAD_DISABLE# to LOW */ //RTK_TODO
		}
	}
...
int board_periph_init(void)
{
	...
	g_InputBufferIoExp0_flag = 0;
	//_s_InputBufferIoExp0_lastVal = dev_pca9535_getInput(IOEXP_I2C_ADDR_U5105_I2C3_0x27, IOEXP_I2C_PORT_U5105_I2C3_0x27);//RTK_TODO
