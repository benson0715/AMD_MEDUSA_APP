...
static const struct device *pwm_1_dev;
static const struct device *tach_0_1_dev;
...
static fan_speed_table_t g_fan_speed_table_init[BOARD_FAN_DEVICE_NUM] = {
	...
	[BOARD_FAN_DEVICE_2] = {                    // For fan device 2
		.profile[FAN_SPEED_TABLE_15W_20W_28W_MODE] = {  // For fan speed table mode
			[FAN_TEMP_TYPE_CPU_TJ] = {
				.tb_type = FAN_TABLE_TYPE_STEPWISE,
				.tb_item = {
...
static fan_ctrl_config_init_t g_fan_ctrl_data_init[BOARD_FAN_DEVICE_NUM] =
{
	...
	[BOARD_FAN_DEVICE_2] = {
		...
		.profile = {
			.location = FAN_LOCATION_RIGHT,
			.tb_select = FAN_SPEED_TABLE_15W_20W_28W_MODE,
			.pTable = (fan_speed_table_t *)&g_fan_speed_table_init[BOARD_FAN_DEVICE_2],
#if (FAN_CTRL_MODE == FAN_RPM_NO_CONTROL) || (FAN_CTRL_MODE == FAN_RPM_MANUAL_WITH_OFFSET_CONTROL || FAN_CTRL_MODE == FAN_RPM_MANUAL_WITH_CURVE_MAP_CONTROL)
			.maxSpeed = 6210,
#endif
			.resolution = FAN_CTRL_PWM_RESOLUTION_L2,
			.ctrl_type = FAN_CTRL_MODE,
			.pCtrl_profile = (control_profile_t *)&s_fan_dev_calibration_init[BOARD_FAN_DEVICE_2],
		},
		/* For debug */
		.overrideTemp = 0,
		.overrideTargetSpeed = 0,
...
static int board_fan_pwm_tach_init(uint8_t pwmCh, uint8_t tachCh)
{
	if (pwmCh == 0) {
		...
		pwm_1_dev =  DEVICE_DT_GET(PWM_2);
		if (!device_is_ready(pwm_1_dev)) {
			LOG_ERR("pwm_1_dev device not ready");
			return -ENODEV;
		};
	} else {
	...
	if (tachCh == 0 || tachCh == 1) {
		tach_0_1_dev =  DEVICE_DT_GET(TACH_n);
		if (!device_is_ready(tach_0_1_dev)) {
			LOG_ERR("tach_0_1_dev device not ready");
			return -ENODEV;
		};
	} else {
...
static int board_fan_tach_rpm_get(uint8_t tachCh)
{
	...
	const struct device *tach_dev = tach_0_1_dev;
