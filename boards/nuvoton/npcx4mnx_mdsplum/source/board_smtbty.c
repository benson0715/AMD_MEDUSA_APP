...
/**
 * @brief BAT LED status set
 *
 * @param     mode:     battery led mode to set
 */
void SET_BAT_LED(bool mode)
{
	ioexp_set(ex_EP_BAT_LEDn, (!mode) ? 1 : 0);

	/* Clear the blink and breathing flag */
	g_batLedBlinkBreathing = 0;
}

/**
 * @brief BAT LED status get
 *
 * @retval bat led status
 */
bool GET_BAT_LED(void)
{
	return ioexp_get(ex_EP_BAT_LEDn) ? false : true;
}
...
{
	/* BAT_LED blink and breathing */
	if (g_batLedBlinkBreathing == 1) {
		g_batLedBlinkBreathing_cnt++;
		/* 2s delay */
		if (g_batLedBlinkBreathing_cnt > 4) {
			g_batLedBlinkBreathing_cnt = 0;
			/* Switch LED status */
			if (GET_BAT_LED())
				SET_BAT_LED(false);
			else
	...
	} else if (g_batLedBlinkBreathing == 2) {
		g_batLedBlinkBreathing_cnt++;
		/* 5s delay */
		if (g_batLedBlinkBreathing_cnt > 10) {
			g_batLedBlinkBreathing_cnt = 0;
			/* Switch BAT LED status */
			if (GET_BAT_LED())
				SET_BAT_LED(false);
			else
				SET_BAT_LED(true);
...
uint32_t board_smtbty_calculateP3tLimit(void)
{
	...
	if (board_pwrSrc_hasAcPowerSource()) {
		uint32_t acLimit = board_pwrSrc_sysAcLimit();
		if (acLimit >= F_AC_ADAPTER_130W_MAX_CURRENT) { /* if it is AC adapter */
			acVoltage = 19;
		} else {                                             /* then treat it as PD adapter */
			acVoltage = 20;
		}

		acBudget = (acLimit * acVoltage * 120 / 100);  /* Peak cap = CurLimit (mA) * Voltage (V) * 120% */
	} else {
		acBudget  = 0;
		acVoltage = 0;
	}
	...
	/**
	 * P3T limit = min(
	 *                  1.125* (Adapter Peak Power + Battery Peak Power - eDP Power (brightness) - USB device being charged Power) ,
	 *                  IRM P3T Limit (i.e. 216W for highest OPN of RMB)
	 *                )
	 *
	 * 1. Adapter Peak Power be treated as 180W as pre Zhang, Tanner comment.
	 * 2. Battery Peak Power will be calculated by following the two map as Samit, Aman suggested.
	 * 3. eDP power will be fixed at 7W and USB device output power will be fixed at 0 to simplify the formula.
	 * 4. IRM P3T Limit will have to rely on the BIOS to EC interface that writes P3T limit to ECRAMxB2
	 */

	if (acBudget + dcBudget > 7000) {
		/* deduct 7000 for eDP power */
		dynamicPowerBudget = acBudget + dcBudget - 7000;
	} else {
		dynamicPowerBudget = 0;
	}
	/* times 1.125 */
		...
		case APP_BATTERY_DISCHARGING:
			if (board_pwrSrc_hasAcPowerSource()) {
				...
				if (u16Data < BOARD_BATTERY_LOW_THRESHOLD) {
					/* Less than 7% */
					//SET_BAT_nLED( BLINKING );
					g_batLedBlinkBreathing = 2;
					g_batLedBlinkBreathing_cnt = 0;
				} else {
					SET_BAT_LED(false);
				}
...
#define CHG_CONST_12800        (12800)
#define CHG_CONST_16000        (16000)
#define CHG_CONST_5000         (5000)
#define CHG_CONST_6000         (6000)
#define CHG_CONST_7890         (F_AC_ADAPTER_150W_MAX_CURRENT)
#define CHG_CONST_16900        (F_AC_ADAPTER_330W_MAX_CURRENT)
...
{
	bool is_330W_AC_Adapter = ioexp_get(ex_AC_330W_PRSNTn) ? (false) : (true);
    uint32_t ac_adapter_current_capability = (is_330W_AC_Adapter) ? (CHG_CONST_16900) : (CHG_CONST_7890);
	uint8_t hasBattery = board_battery_isPresent(0);
	BOARD_PWR_SRC_TYPE pwrSrc = board_pwrSrc_getCurrentPowerSource();

	// uint32_t sysAcLimit = board_pwrSrc_sysAcLimit();
	...
	uint32_t acLimit1       = CHG_CONST_12800;      // PD_Adapter default: CHG_CONST_5000
	uint32_t acLimit2       = CHG_CONST_12800;      // PD_Adapter default: CHG_CONST_5000
	uint32_t AcProchot      = CHG_CONST_16000;      // PD_Adapter default: CHG_CONST_6000
	...
	if (AcDcSwitchEnableFlag) {
		...
		if (hasBattery) {
			...
			tmp1 = board_pwrSrc_AcCurLimit();
			tmp2 = board_pwrSrc_PdCurLimit();
			if (pwrSrc == BOARD_PWR_AC) {
				if (tmp1) {
					acLimit1 = tmp1;
					acLimit2 = PERCENTAGE(tmp1, 130);
				}
			} else if (pwrSrc == BOARD_PWR_PD){
				if (tmp2) {
					acLimit1 = tmp2;
					acLimit2 = PERCENTAGE(tmp2, 130);
				}
			}
			...
			if (g_isThrottleApuInAcOnly) {
				...
				if (pwrSrc == BOARD_PWR_AC) {
					if (ac_attached) {
						AcProchot = PERCENTAGE(tmp1, 95);
					} else {
						tmp1 = PERCENTAGE(tmp1, 120);
						AcProchot = DcProchotLevel < tmp1 ? DcProchotLevel : tmp1;
					}
				}else if (pwrSrc == BOARD_PWR_PD) {
					if (ac_attached) {
						AcProchot = PERCENTAGE(tmp2, 95);
					} else {
						tmp2 = PERCENTAGE(tmp2, 120);
						AcProchot = DcProchotLevel < tmp2 ? DcProchotLevel : tmp2;
					}
				}
		...
		switch (pwrSrc) {
	...
	#if (CONFIG_USBC_CCGX)
					amd_crb_drivers_hpi_snkCapUnconstrainedPower(TYPEC_PORT_0_IDX, 0, false);
					amd_crb_drivers_hpi_snkCapUnconstrainedPower(TYPEC_PORT_1_IDX, 0, false);
					amd_crb_drivers_hpi_srcCapUnconstrainedPower(TYPEC_PORT_0_IDX, 0, false);
					amd_crb_drivers_hpi_srcCapUnconstrainedPower(TYPEC_PORT_1_IDX, 0, false);
	#if PD_TRIPPORT_ENABLE
					amd_crb_drivers_hpi_snkCapUnconstrainedPower(TYPEC_PORT_0_IDX, 1, false);
					amd_crb_drivers_hpi_srcCapUnconstrainedPower(TYPEC_PORT_0_IDX, 1, false);
	#endif
	#endif
				}
			...
			case BOARD_PWR_AC:
				if (hasBattery) {                              /* AC + Battery */
					acLimit1 = PERCENTAGE(ac_adapter_current_capability, 90);
					acLimit2 = PERCENTAGE(ac_adapter_current_capability, 130);
					AcProchot = PERCENTAGE(ac_adapter_current_capability, 95);
					/* control PSYS AC+DC : charger on*/
					dev_isl9241_regRMW(DEV_CHARGER_CURRENT_PORT, DEV_ISL9241_REG_Control1, cbit(3), cbit(3));

					LOG_DBG ("SetChager: AC + Battery");
				} else if (g_isThrottleApuInAcOnly) {          /* AC only + Prochot EN */
					acLimit1 = PERCENTAGE(ac_adapter_current_capability, 90);
					acLimit2 = PERCENTAGE(ac_adapter_current_capability, 130);
					AcProchot = PERCENTAGE(ac_adapter_current_capability, 95);

					/* control PSYS AC only: charger on*/
					dev_isl9241_regRMW(DEV_CHARGER_CURRENT_PORT, DEV_ISL9241_REG_Control1, cbit(3), cbit(3));

					LOG_DBG ("SetChager: AC only + ACProchotL %dmA", AcProchot);
				} else {                                       /* AC only + Prochot OFF */
					/* Leave ACLimit 1/2 at MAX. */
				}

				if (CHECK_CFG(PD0, CCG8DF) || CHECK_CFG(PD1, CCG8SF)) {
#if (CONFIG_USBC_CCGX)
					amd_crb_drivers_hpi_snkCapUnconstrainedPower(TYPEC_PORT_0_IDX, 0, true);
					amd_crb_drivers_hpi_snkCapUnconstrainedPower(TYPEC_PORT_1_IDX, 0, true);
					amd_crb_drivers_hpi_srcCapUnconstrainedPower(TYPEC_PORT_0_IDX, 0, true);
					amd_crb_drivers_hpi_srcCapUnconstrainedPower(TYPEC_PORT_1_IDX, 0, true);
#if PD_TRIPPORT_ENABLE
					amd_crb_drivers_hpi_snkCapUnconstrainedPower(TYPEC_PORT_0_IDX, 1, true);
					amd_crb_drivers_hpi_srcCapUnconstrainedPower(TYPEC_PORT_0_IDX, 1, true);
#endif
#endif
				}
			...
			case BOARD_PWR_PD:
				if (hasBattery) {                              /* PD + Battery */
					acLimit1 = PERCENTAGE(board_pwrSrc_PdCurLimit(), 90);
					acLimit2 = PERCENTAGE(acLimit1, 130);
					AcProchot = PERCENTAGE(board_pwrSrc_PdCurLimit(), 95);
				...
				/* force prochot duration to 15ms for PD source only. because PD source is lower than AC */
				prochotDuration = 2;
				...
				if (amd_crb_drivers_raa489300_ready()) {
					if (pdo.fixed_src.voltage > 560) {  /* VBUS Voltage > 28V */
						/* voltage above 28V */
						amd_crb_drivers_raa489300_init(0);
					} else {
						/* voltage above 20V ~ 28V */
						amd_crb_drivers_raa489300_init(1);
					}
				}
#endif
