 ...
 *
 * @note
 *   DAC event handler for processing DAC table updates via I2C
 */
void board_dac_event_handler (void)
{
		uint8_t option_val = 0xFF;
		...
		for(uint8_t i = 0; g_dac_Table[i].REG != 0xFF; i++){
				if (g_dac_Table[i].active){
					...
					if(g_dac_Table[i].receive_value == option_val){
						...
						LOG_DBG("Write PMIC/vdd_mem DAC the same value on I2C, skip [%02X]", g_dac_Table[i].receive_value);
						continue;
					}			
					...
					if(u8data == 0xFF){
						...
						LOG_DBG("Write PMIC/vdd_mem _s_vdd_mem_val out of list on I2C [%02X]", g_dac_Table[i].receive_value);
						continue;
					}
...
void board_dac_restore_setting(void)
{
	...
	for(uint8_t i = 0; g_dac_Table[i].REG != 0xFF; i++){
		...
		/* I2C programming to make it effective */
		ret = i2c_hub_burst_write_multi_cmd(I2C_DAC_PWR_PORT, g_dac_Table[i].I2C_Address, &(g_dac_Table[i].REG), 1, &u8data, 1);
		LOG_DBG("VDDP_MEM		 ... ret %d vdd_mem = 0x%02X reg 0x%02X = 0x%02X%s", ret, option_val, g_dac_Table[i].REG, u8data, (ret != 0) ? ", but !!ERROR!!" : "");
	}
}
