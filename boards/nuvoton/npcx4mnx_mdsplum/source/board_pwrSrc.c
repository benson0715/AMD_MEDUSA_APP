 ...
 *          Macro             *
 * ************************** */
#define BOARD_PWRSRC_PD_DEFAULT_CURRENT 2250   /* 2250mA */
 ...
 *     global valuable        *
 * ************************** */
bool g_AcAdapterConnected = false;         /* debounced AC_POWER_PRSNT_ECn */
...
#if PD_DC_HOTPLUG_PROCHOT
/**
 * @brief Timer callback for DC prochot
 *
...
#if PD_AC_HOTPLUG_PROCHOT
/**
 * @brief Timer callback for AC prochot
 *
 * @param timer Pointer to the timer structure
 */
static void _pdAcHotplugProchot_TimerCallback(struct k_timer *timer)
...
/**
 * @brief Clear AC adapter power capability
 */
void board_pwrSrc_clearAcCap (void)
{
	int key = irq_lock();
	_s_pwrCap[AC_SINK].CurLimit = 0;
	_s_pwrCap[AC_SINK].VadpVoltage = 0;
	irq_unlock(key);
}

/**
 * @brief Clear PD adapter power capability
 */
...
{
	return (g_PdAdapterConnected | g_AcAdapterConnected);
}
...
{
	return _s_pwrCap[AC_SINK].CurLimit + _s_pwrCap[PD_SINK].CurLimit;
}
...
/**
 * @brief return the charger AC current limit
 *
 * @retval AC current limit
 */
uint32_t board_pwrSrc_AcCurLimit(void)
{
	return _s_pwrCap[AC_SINK].CurLimit;
}
/**
...
{
	uint32_t ret = 0;
	uint32_t acPwrMaxWatt = _s_pwrCap[AC_SINK].CurLimit * _s_pwrCap[AC_SINK].VadpVoltage;
...
/**
 * @brief control the PD sink enable/disable. True: enable/False: disable
 *
 * @param enable True to enable PD sink, False to disable
 */
static void board_pwrSrcPdAdapter(bool enable)
{
	if (enable)
		/* EC_AC_PDn low enable PD */
		gpio_write_pin(EC_AC_PD_N, 0);
	else
		/* EC_AC_PDn high disable PD */
		gpio_write_pin(EC_AC_PD_N, 1);
}


/**
 * @brief return EC_AC_PD status. High: PD adapter is gated off
 *
 * @retval True if PD adapter is gated off
 */
static bool board_pwrSrcIsPdAdapterGateOff(void)
{
	/* HIGH means PD Adapter is gated off */
	return gpio_read_pin(EC_AC_PD_N);
}
...
void board_pwrSrc_powerOnReset(void)
{
	...
	/* enable PD sink by default */
	if (board_pwrSrcIsPdAdapterGateOff())
		board_pwrSrcPdAdapter(true);
	...
	/* 02. AC adapter */
	if (!gpio_read_pin(AC_PRSNT_N)) {
		g_AcAdapterConnected = true;
		_s_emMainPwrSource = BOARD_PWR_AC;
		/* Get the 330W AC adapter attached status low active */
		g_ac330Prest = ioexp_get(ex_AC_330W_PRSNTn);
		/* g_ac330Prest = 0: means 330W adatper plugin */
		if (g_ac330Prest) {
			_s_pwrCap[AC_SINK].CurLimit    = F_AC_ADAPTER_150W_MAX_CURRENT;
			_s_pwrCap[AC_SINK].VadpVoltage = F_AC_ADAPTER_VIN_VOLTAGE;
		} else {
			_s_pwrCap[AC_SINK].CurLimit    = F_AC_ADAPTER_330W_MAX_CURRENT;
			_s_pwrCap[AC_SINK].VadpVoltage = F_AC_ADAPTER_VIN_VOLTAGE_330;
		}
	} else {
		board_pwrSrc_clearAcCap();
		g_AcAdapterConnected = false;
	}
	/* 03. PD adapter */
	if (!gpio_read_pin(PD_PRSNT_N)) {
...
#if PD_MODULE_48V_EN
			/* need to adjust the current for 48V PD. There is one additional regulator to convert 28~48V to 20V and input to mother board */
			if (pdo.fixed_src.voltage > 560) {  /* 560*50 = 28000mV */
				_s_pwrCap[PD_SINK].CurLimit    = (pdo.fixed_src.max_current * 10 * pdo.fixed_src.voltage) / 400;  /* assume the regulator efficiency is 100% */
			}
#endif
		} else {
		...
		if (_s_emMainPwrSource == BOARD_PWR_AC) {
			/* There are hardware circuit to make sure AC+PD use AC as source */
			_s_emMainPwrSource = BOARD_PWR_AC;
	 ...
	 */
	if (!g_AcAdapterConnected && !g_batPresentFlag) {
	...
	LOG_DBG("        Main Source: %s", (_s_emMainPwrSource == BOARD_PWR_AC) ? "AC Brick" : ((_s_emMainPwrSource == BOARD_PWR_PD) ? "PD Adapter" : ((_s_emMainPwrSource == BOARD_PWR_BTY) ? "Battery" : "unknown")) );
	LOG_DBG(" Sys totoal Cur Lmt: %d", board_pwrSrc_sysAcLimit());
	LOG_DBG("     AC-apd Cur Lmt: %d", board_pwrSrc_AcCurLimit());
	...
	LOG_DBG("  AC_PWR = %d; PD_PWR = %d; BATY = %d; ACOK = %d", g_AcAdapterConnected, g_PdAdapterConnected, g_batPresentFlag, g_ChgAcOK);
}
...
static uint32_t _pwrSrc_stateSnapshot(void)
{
	...
	uint32_t helper = (xor << 16)                             |
		                (_s_emMainPwrSource & 0x0F)             |
										...
										(g_ChgAcOK                ? 0x0020 : 0) |
										(g_AcAdapterConnected     ? 0x0040 : 0) |
										(g_PdAdapterConnected     ? 0x0080 : 0) |
										(ioexp_get(ex_EP_ADAPTER_OFF) ? 0x0100 : 0);
...
static void board_pwrSrc_stateMonitor (void)
{
	...
	board_smtbty_ChgDisable();
		...
		/* Select the higher cap type-c port */
		if ((g_PdAdapterConnected && g_batPresentFlag) ||
			(g_AcAdapterConnected && g_PdAdapterConnected)) {
			/* battery plugin and switch to highest cap */
			/* ac plugin and switch to highest cap. Because HW will auto switch to ac power source*/
			if (app_usbc_getAcLimitOfHighestPdPort()) {
					/* Cannot access PD I2C in this task. It may cause I2C conflict */
					g_ccgxUpdateSinkPath = 1;
					/* Signal USBC task */
					app_usbc_giveSem();
			}
		}
	...
	extern uint8_t g_batPresentFromPin;
	/* Identify the power source - start */
	if (g_batPresentFromPin && AcDcSwitchEnableFlag && !g_ChgAcOK) {
		LOG_ERR("Fake DC + dead battery/damaged board case ...");
		if (g_AcAdapterConnected) {
			/* g_ac330Prest = 0: 330W adapter plugin */
			g_ac330Prest = ioexp_get(ex_AC_330W_PRSNTn);
			/* g_ac330Prest = 0: means 330W adatper plugin */
			if (g_ac330Prest) {
				_s_pwrCap[AC_SINK].CurLimit    = F_AC_ADAPTER_150W_MAX_CURRENT;
		...
		_s_emMainPwrSource = BOARD_PWR_AC_AND_PD;
	} else if (g_batPresentFlag && !g_ChgAcOK) {
		_s_emMainPwrSource = BOARD_PWR_BTY;
		board_pwrSrc_clearAcCap();
		board_pwrSrc_clearPdCap();
	} else if (g_ChgAcOK) {
		...
		if (g_AcAdapterConnected && g_PdAdapterConnected) {
			/* There are hardware circuit to make sure AC+PD use AC as source */
			_s_emMainPwrSource = BOARD_PWR_AC;
		} else if (g_AcAdapterConnected) {
			_s_emMainPwrSource = BOARD_PWR_AC;
			board_pwrSrc_clearPdCap();
		} else if (g_PdAdapterConnected) {
			_s_emMainPwrSource = BOARD_PWR_PD;
#if PD_AC_HOTPLUG_PROCHOT
			if ((oldMainPwrSource == BOARD_PWR_AC) &&
				(inS0 == 1)) {
				/* AC switch to PD case */
				gpio_write_pin(EC_APU_PROCHOT_N, 0);
				k_timer_start(&_s_pdAcHotplugProchotTid, K_MSEC(2000), K_NO_WAIT);
			} else {
...
/**
 * @brief Handle the power source event after delay
 */
static void board_pwrSrc_delayCallback(void)
{
	LOG_DBG("  ---------------------");
	LOG_DBG("        Main Source: %s", (_s_emMainPwrSource == BOARD_PWR_AC) ? "AC Brick" : ((_s_emMainPwrSource == BOARD_PWR_PD) ? "PD Adapter" : ((_s_emMainPwrSource == BOARD_PWR_BTY) ? "Battery" : "AC and PD")));
	LOG_DBG(" Sys totoal Cur Lmt: %d", board_pwrSrc_sysAcLimit());
	LOG_DBG("     AC-apd Cur Lmt: %d mA", board_pwrSrc_AcCurLimit());
...
/**
 * @brief Power source firmware status handler
 *
 * @param isRead Read or write operation flag
 * @param ui8Idx Index of the data
 * @param pui8Data Pointer to the data buffer
 *
 	...
 	if (isRead) {
 		switch (ui8Idx) {
 			...
 			case 0x10: case 0x11: case 0x12: case 0x13:
				*pui8Data = *(((uint8_t *)&_s_pwrCap[AC_SINK].CurLimit) + (ui8Idx - 0x10));
				break;
			case 0x14: case 0x15: case 0x16: case 0x17:
				*pui8Data = *(((uint8_t *)&_s_pwrCap[AC_SINK].VadpVoltage) + (ui8Idx - 0x14));
				break;
