 ...
 *          Macro             *
 * ************************** */
#define BOARD_PWRSRC_PD_DEFAULT_CURRENT 2000   /* 2000mA */
...
void board_pwrSrc_powerOnReset(void)
{
	...
	if (!g_batPresentFlag) {
		...
		if (!g_PdAdapterConnected) {
			...
			g_PdAdapterConnected = true;
			_s_emMainPwrSource = BOARD_PWR_PD;
			board_smtbty_setAcLimit(DEV_CHARGER_CURRENT_PORT, board_pwrSrc_PdCurLimit(), (board_pwrSrc_PdCurLimit() * 120 / 100));
	...
	/* battery only */
	if (_s_emMainPwrSource == BOARD_PWR_BTY) {
	...
	} else if (_s_emMainPwrSource == BOARD_PWR_PD) {
		/* AC */
		if (g_batPresentFlag) {
			/* AC + battery */
	...
	/* set the full charge voltage to 8.8V */
	u16tmp = APP_BATTERY_FULL_CHARGE_VOLTAGE;
	chgAck = amd_crb_drivers_mp2764_fullChargeVoltage(DEV_CHARGER_CURRENT_PORT, (u16tmp >> 1)); /* two cells. APP_BATTERY_FULL_CHARGE_VOLTAGE/2 */
	if (!chgAck) {
		LOG_DBG("[ERROR] Fail to write MaxSystemVoltage to 0.008V!");
	}

	/* set the input min voltage */
	u16tmp = 8000;
	chgAck = amd_crb_drivers_mp2764_VinMinVoltage(DEV_CHARGER_CURRENT_PORT, u16tmp);  /* Vin Min voltage 8V */
	if (!chgAck) {
		LOG_DBG("[ERROR] Fail to write VinVoltage to 8000mV!");
	}

	/* mask the interrupt event */
	chgAck = amd_crb_drivers_mp2764_interruptMask(DEV_CHARGER_CURRENT_PORT, 0);
	if (!chgAck) {
		LOG_DBG("[ERROR] Fail to clear the intMask!");
	}

	/* Config0:
	 * enable -> IIN_LIM_EN */
	chgAck = amd_crb_drivers_mp2764_regRMW(DEV_CHARGER_CURRENT_PORT, DEV_MP2764_REG_ConfigReg0, 0x0040, 0x0040);
	if (!chgAck) {
		LOG_DBG("[ERROR] Fail to set DEV_MP2764_REG_ConfigReg0!");
	}

	/* Config1:
	 * enable -> discharge OCP 12A
	 * enable -> discharge OCP debounce 100ms
	 * disable -> low power mode*/
	chgAck = amd_crb_drivers_mp2764_regRMW(DEV_CHARGER_CURRENT_PORT, DEV_MP2764_REG_ConfigReg1, 0x0010, 0x2018);
	if (!chgAck) {
		LOG_DBG("[ERROR] Fail to set DEV_MP2764_REG_ConfigReg1!");
	}
	 ...
	 * enable -> TWO_INLIM_EN
	 * disable -> WDT  */
	chgAck = amd_crb_drivers_mp2764_regRMW(DEV_CHARGER_CURRENT_PORT, DEV_MP2764_REG_ConfigReg2, 0x0008, 0x1C08);
	if (!chgAck) {
		LOG_DBG("[ERROR] Fail to set DEV_MP2764_REG_ConfigReg2!");
	}

	/* Config3: no need to change */

	/* Config4:
	 * enable -> PSYS AC is enabled automatically, need to enable PSYS for DC mode */
	chgAck = amd_crb_drivers_mp2764_regRMW(DEV_CHARGER_CURRENT_PORT, DEV_MP2764_REG_ConfigReg4, 0x0002, 0x0002);
	if (!chgAck) {
		LOG_DBG("[ERROR] Fail to set DEV_MP2764_REG_ConfigReg4!");
	}

	/* Config5:
	 * enable -> BAT_NUM 2cell
	 * enable -> IBATT_RESENSE_SEL 10mohm
	 * enable -> IIN_RESENSE_xSEL 10mohm
	 * disable -> IN_GAIN
	 * disable -> IBATT_GAIN */
	chgAck = amd_crb_drivers_mp2764_regRMW(DEV_CHARGER_CURRENT_PORT, DEV_MP2764_REG_ConfigReg5, 0x0000, 0x018F);
	if (!chgAck) {
		LOG_DBG("[ERROR] Fail to set DEV_MP2764_REG_ConfigReg5!");
	}

	/* Disable unused prochot source */
	chgAck = amd_crb_drivers_mp2764_regRMW(DEV_CHARGER_CURRENT_PORT, DEV_MP2764_REG_ProchotConfig2, 0x0000, 0x01C7);
	if (!chgAck) {
		LOG_DBG("[ERROR] Fail to set DEV_MP2764_REG_ProchotConfig2!");
	}
...
static void board_pwrSrc_delayCallback(void)
{
	...
	if (BOARD_PWR_BTY == _s_emMainPwrSource) {
		...
		chgAck = amd_crb_drivers_mp2764_setBgate(DEV_CHARGER_CURRENT_PORT, 1);
		if (!chgAck) {
		...
		if (g_isThrottleApuInDcOnly) {
			...
			amd_crb_drivers_mp2764_batteryOnly(DEV_CHARGER_CURRENT_PORT, true);
		}
	} else {
		amd_crb_drivers_mp2764_batteryOnly(DEV_CHARGER_CURRENT_PORT, false);
		amd_crb_drivers_mp2764_setBgate(DEV_CHARGER_CURRENT_PORT, 1);
	}
	/**
