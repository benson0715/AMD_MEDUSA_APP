...
extern uint8_t g_triggerKbcRst;
...
static int process_keyboard_command(uint8_t command, uint8_t *output)
{
	...
	case KBC_8042_PULSE_OUTPUT:
		/* trigger KBC reset for KBC command from 0xFE */
		g_triggerKbcRst = 1;
...
/**
 * Handle data transfer between host and EC.
 *
 * @param host_data Structure containing data and command/data flag from host.
 */
...
/**
 * Thread function to handle data from host and initialize KBC.
 *
...
#if CONFIG_KSCAN_EC
	espihub_add_S0i3SelfRestore_handler(S0i3SelfRestore);
#endif
	while (true) {
		...
		/* handle the KBC reset pin if need */
		board_init_kbcRst();
	}
}

/**
 * Thread function to send keyboard data to host.
 *
 * @param p1 Unused parameter.
 * @param p2 Unused parameter.
 * @param p3 Unused parameter.
 ...
 *
 * @param data Data byte received from PS2 keyboard.
 */
 ...
 *
 * @param data Data byte received from PS2 mouse.
 */
...
#if defined(CONFIG_KSCAN_EC)
/**
 * Callback for matrix keyboard scan data.
 *
 * @param data Pointer to keyboard scan data buffer.
...
/**
 * Initialize KBC and PS2 devices.
 *
...
/**
 * Disable KBC interface and associated devices.
 *
 * @return 0 on success, negative error code on failure.
 */
...
/**
 * Enable KBC interface and associated devices.
 *
 * @return 0 on success, negative error code on failure.
 */
...
/**
 * Re-init 8042KBC and sync up IRQ1 status with host.
 * It can cause the OS thinks wakeup event is KB if VW is re-init at
 * very early S0i3 resume phase.
 *
 * @return 0 on success, negative error code on failure.
 */
uint8_t S0i3SelfRestore(void)
{
	...
	/* This proc is needed only if eSPI bus is reset in S0i3 */
	if (m_espi_ResetInS0i3) {
#if CONFIG_SLEEP
		extern uint8_t g_ui8MondernStandbySupport;
		extern uint8_t g_ui8isKbcWakeEvent;
		LOG_INF("VW_Enable: MsFlag %d, KbcWakeEvt %d", g_ui8MondernStandbySupport, g_ui8isKbcWakeEvent);
		if (g_ui8MondernStandbySupport && 1 == g_ui8isKbcWakeEvent) {
			send_kb_to_host(0x65);
			/* Sync up SIRQ1 status of APU side */
			ret = espihub_kbc_write(E8042_PAUSE_IRQ, 0);
			...
			g_ui8isKbcWakeEvent = 0;
			LOG_INF("VW_Enable: set KbcWakeEvt = %d", g_ui8isKbcWakeEvent);
		}
#endif
	}
	return ret;
...
/**
 * Handle the configuration data from the host for both keyboard and mouse.
 *
 * @param data Data byte from host.
 * @param cmd_data Command/data flag (1 = command, 0 = data).
 */
void kbc_handler(uint8_t data, uint8_t cmd_data)
{
	...
	/* Guard against ISR re-trigger delivering the same byte twice.
	 * Only suppress if both data AND cmd_data match the previous
	 * invocation, so that legitimate repeated host commands
	 * (same data value but different cmd/data phase, or two
	 * intentionally identical writes) are not dropped.
	 */
	if (has_prev && last_data == data && last_cmd == cmd_data) {
		/* Consume the duplicate but reset so a third identical
		 * byte (which would be a real host retry) goes through.
		 */
		has_prev = false;
		return;
	}
...
/**
 * Get current keyboard LED states.
 *
...
/**
 * Send keyboard data byte to host queue.
 *
 * @param data Data byte to send to host.
 */
static void send_kb_to_host(uint8_t data)
{
	/**
	 * 0x65 is a special data, source from amd crb keil ec behavior.
	 * It can cause the OS thinks wakeup
	 * event is KB.
	 */
	uint32_t optionVal;
...
/**
 * Purge all pending data from keyboard queue.
 */
static void purge_kb_queue(void)
{
