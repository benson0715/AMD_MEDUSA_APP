...
LOG_MODULE_REGISTER(tps6699x, CONFIG_TPS_LOG_LEVEL);
...
uint8_t g_tps6699x_prev_4CC[NO_OF_TYPEC_PORTS][TIPD_REG_CMD1_LEN] = {0};
uint8_t g_tps6699x_prev_Data[NO_OF_TYPEC_PORTS][TIPD_MAX_PACKAGE_LENGTH] = {0};
uint8_t g_tps6699x_prev_len[NO_OF_TYPEC_PORTS] = {0};
...
/**
 * amd_crb_drivers_tps6699x_regAccess
 * 
...
/**
 * amd_crb_drivers_tps6699x_burst_patch_download
 * 
...
/**
 * amd_crb_drivers_tps6699x_wait4CmdDone
 *
...
uint32_t amd_crb_drivers_tps6699x_wait4CmdDone (uint8_t port, uint32_t ms)
{
	...
	if (ms) {
		do {
			amd_crb_drivers_tps6699x_regAccess(TI_REG_READ, TIPD_REG_CMD1, cmdBuf, (TIPD_REG_CMD1_LEN + 1), port);
...
/**
 * amd_crb_drivers_tps6699x_cmdExecutor
 *
...
uint32_t amd_crb_drivers_tps6699x_cmdExecutor(uint32_t timeout, uint8_t port, uint8_t * pDataIn, uint8_t dataInLen, uint8_t * p4CC, uint8_t * pDataOut, uint8_t dataOutLen)
{
	...
	/* 1. CMD is 0 or '!CMD' */
	amd_crb_drivers_tps6699x_wait4CmdDone(port, timeout);
	...
	if ((dataInLen != 0) && (pDataIn != NULL)) {
		...
		g_tps6699x_prev_len[port] = dataInLen;
		for (index = 0; index < dataInLen; index++) {
			g_tps6699x_prev_Data[port][index] = pDataIn[index];
		}

		amd_crb_drivers_tps6699x_regAccess(TI_REG_WRITE, TIPD_REG_DATA1, tmpBuf, (dataInLen + 1), port);
	}
	else {
		g_tps6699x_prev_len[port] = 0;
	}
	...
	if (p4CC) {
		...
		for (index = 0; index < 4; index++) {
			g_tps6699x_prev_4CC[port][index] = p4CC[index];
		}

		amd_crb_drivers_tps6699x_regAccess(TI_REG_WRITE, TIPD_REG_CMD1, tmpBuf, (TIPD_REG_CMD1_LEN + 1), port);
	}

	/* 4. wait until CMD changes to 0 or '!CMD' */
	u32rsp = amd_crb_drivers_tps6699x_wait4CmdDone(port, timeout);
	...
	if (pDataOut && dataOutLen) {
		tmpBuf[0] = 0;
		ret = amd_crb_drivers_tps6699x_regAccess(TI_REG_READ, TIPD_REG_DATA1, tmpBuf, (dataOutLen + 1), port);
		...
		LOG_DBG("%s success without output P:%d", p4CC, port);
		return 0; /* success get the resp */
	}
...
/**
 * amd_crb_drivers_tps6699x_readSnkRdo
 *
 * @param [in]     port;       port for ti pd
...
{
	DRV_TPS6699X_ACTIVE_CONTRACT_RDO actRdo;
	...
	if ((dpm_get_info(port)->attach) &&
		...
		int ret = amd_crb_drivers_tps6699x_regAccess(TI_REG_READ, TIPD_REG_ACTIVE_RDO, &actRdo, sizeof(actRdo), port);
...
/**
 * amd_crb_drivers_tps6699x_readSrcRdo
 *
 * @param [in]     port;       port for ti pd
 * @return 0 If no contract.
...
/**
 * amd_crb_drivers_tps6699x_readSnkPdo
 *
 * @param [in]     port;       port for ti pd
 * @return 0 If no contract.
...
{
	DRV_TPS6699X_ACTIVE_CONTRACT_PDO actPdo;
	...
	if ((dpm_get_info(port)->attach) &&
		//(dpm_get_info(port)->contract_exist) &&
		(dpm_get_info(port)->cur_port_role == PRT_ROLE_SINK)) {
		int ret = amd_crb_drivers_tps6699x_regAccess(TI_REG_READ, TIPD_REG_ACTIVE_PDO, &actPdo, sizeof(actPdo), port);
...
/**
 * amd_crb_drivers_tps6699x_gpioConfig
 *
 * @param [in]     port;          port for ti pd
 * @param [in]     pinMask;       pin mask to change
 * @param [in]     output;        enable pin output
 * @return 0 If successful.
 * @return -EIO General input / output error.
 *
 * @note
 *  config the ti pd gpio0 status
 */
int amd_crb_drivers_tps6699x_gpio0Output(uint8_t port, uint16_t pinMask, uint16_t output)
{
	DRV_TPS6699X_GPIO_P0 config;
	int ret = 0;

	LOG_DBG("GPIO_Config P:%d", port);

	/* Read the whole gpio status */
	ret = amd_crb_drivers_tps6699x_regAccess(TI_REG_READ, TIPD_REG_GPIO_P0, &config, sizeof(config), port);

	config.f.OutputEnable |= pinMask;  /* enable the output for mask pin */
	config.f.OutputOdEnable &= ~pinMask;  /* disable output OD for mask pin */
	config.f.Output_Data &= ~pinMask;
	config.f.Output_Data |= (output & pinMask);  /* change output data */

	/* Write the output enable to gpio status */
	ret = amd_crb_drivers_tps6699x_regAccess(TI_REG_WRITE, TIPD_REG_GPIO_P0, &config, sizeof(config), port);
	/* Read the whole gpio status */
	ret = amd_crb_drivers_tps6699x_regAccess(TI_REG_READ, TIPD_REG_GPIO_P0, &config, sizeof(config), port);
...
{
	DRV_TPS6699X_GPIO_P1 config;
	int ret = 0;

	LOG_DBG("GPIO_Config P:%d", port);

	/* Read the whole gpio status */
	ret = amd_crb_drivers_tps6699x_regAccess(TI_REG_READ, TIPD_REG_GPIO_P1, &config, sizeof(config), port);
	...
	/* Write the output enable to gpio status */
	ret = amd_crb_drivers_tps6699x_regAccess(TI_REG_WRITE, TIPD_REG_GPIO_P1, &config, sizeof(config), port);
	/* Read the whole gpio status */
	ret = amd_crb_drivers_tps6699x_regAccess(TI_REG_READ, TIPD_REG_GPIO_P1, &config, sizeof(config), port);
...
/**
 * amd_crb_drivers_tps6699x_isUsb4Enter
 *
 * @param [in]     port;          port for ti pd
...
{
	DRV_TPS6699X_USB_STATUS status;
	int ret = 0;

	LOG_DBG("USB_STATUS P:%d", port);

	/* Read the USB config */
	ret = amd_crb_drivers_tps6699x_regAccess(TI_REG_READ, TIPD_REG_USB_STS, &status, sizeof(status), port);

    /*  0h = No Enter_USB. Enter_USB has not been sent as DFP
		1h = Enter_USB timeout. The UFP should have received an
		2h = Enter_USB failure. In DFP mode the transmitted En
		3h = Successful Enter_USB. An Enter_USB was transmitted
	*/
	if (status.f.eudoSopSendRecv == 3)
		return true;
	else
		return false;
}
 ...
 *
 * @param [in]     port;          port for ti pd
 * @return true: enter DP alt mode
 *
 * @note
 *  return if ti pd enter DP alt mode
 */
bool amd_crb_drivers_tps6699x_isDpEnter(uint8_t port)
{
	DRV_TPS6699X_DP_STATUS status;
	int ret = 0;

	LOG_DBG("DP_STATUS P:%d", port);

	/* Read the USB config */
	ret = amd_crb_drivers_tps6699x_regAccess(TI_REG_READ, TIPD_REG_DP_SID_STATUS, &status, sizeof(status), port);

	if (status.f.dpModeActive)
		return true;
	else
		return false;
}
...
{
	DRV_TPS6699X_INTEL_VID_STATUS status;
	int ret = 0;

	LOG_DBG("VID_STATUS P:%d", port);

	/* Read the USB config */
	ret = amd_crb_drivers_tps6699x_regAccess(TI_REG_READ, TIPD_REG_INTEL_VID_STATUS, &status, sizeof(status), port);

	if (status.f.tbtModeActive)
		return true;
	else
		return false;
}
...
/**
 * amd_crb_drivers_tps6699x_txVidUpdate
 *
 * @param [in]     port;           port for ti pd
...
{
	DRV_TPS6699X_TX_VID vid;
	int ret = 0;

	LOG_DBG("TIPD_REG_TX_IDENTITY P:%d", port);

	/* Read the TX vid */
	ret = amd_crb_drivers_tps6699x_regAccess(TI_REG_READ, TIPD_REG_TX_IDENTITY, &vid, sizeof(vid), port);
	...
	/* Write TX vid */
	ret = amd_crb_drivers_tps6699x_regAccess(TI_REG_WRITE, TIPD_REG_TX_IDENTITY, &vid, sizeof(vid), port);

	return ret;
}

/**
 * amd_crb_drivers_tps6699x_txVidChangeModelOpSupp
 *
 * @param [in]     port;           port for ti pd
...
int amd_crb_drivers_tps6699x_txVidChangeModelOpSupp(uint8_t port, bool ModelOpSupp)
{
	...
	ret = amd_crb_drivers_tps6699x_regAccess(TI_REG_READ, TIPD_REG_TX_IDENTITY, &vid, DRV_TPS6699X_TX_VID_REGISTER_LENGTH, port);
	/* Change the model operation support bit */
	vid.f.ModalOperationSupported = ModelOpSupp;

	/* Write TX vid */
	ret = amd_crb_drivers_tps6699x_regAccess(TI_REG_WRITE, TIPD_REG_TX_IDENTITY, &vid, DRV_TPS6699X_TX_VID_REGISTER_LENGTH, port);
	ret = amd_crb_drivers_tps6699x_regAccess(TI_REG_READ, TIPD_REG_TX_IDENTITY, &vid, DRV_TPS6699X_TX_VID_REGISTER_LENGTH, port);
	return ret;
}

/**
 * amd_crb_drivers_tps6699x_txVidOverwrite
 *
 * @param [in]     port;           port for ti pd
 * @return 0 If successful.
 * @return -EIO General input / output error.
...
/**
 * amd_crb_drivers_tps6699x_setSxAppConfig
 *
 * @param [in]     port;           port for ti pd
...
{
	DRV_TPS6699X_SET_SX_APP_CONFIG config;
	int ret = 0;

	LOG_DBG("TIPD_REG_SET_SX P:%d", port);

	/* Read the TX vid */
	ret = amd_crb_drivers_tps6699x_regAccess(TI_REG_READ, TIPD_REG_SX_APP_CONFIG, &config, sizeof(config), port);
	...
	/* Write TX vid */
	ret = amd_crb_drivers_tps6699x_regAccess(TI_REG_WRITE, TIPD_REG_SX_APP_CONFIG, &config, sizeof(config), port);

	return ret;
}

/**
 * amd_crb_drivers_tps6699x_bootStatus
 *
 * @param [in]     port;           port for ti pd
 *
 * @note
...
{
	int ret = 0;
	DRV_TPS6699X_BOOT_FLAGS status;
	...
	ret = amd_crb_drivers_tps6699x_regAccess(TI_REG_READ, TIPD_REG_BOOT, &status, sizeof(status), port);
}

/**
 * amd_crb_drivers_tps6699x_sleepConfig
 *
 * @param [in]     port;           port for ti pd
...
{
	int ret = 0;
	DRV_TPS6699X_SLEEP_CONFIG sleep_config;
	...
	/* read current port control */
	ret = amd_crb_drivers_tps6699x_regAccess(TI_REG_READ, TIPD_REG_SLEEP_CONFIG, &sleep_config, sizeof(sleep_config), port);
	...
	amd_crb_drivers_tps6699x_regAccess(TI_REG_WRITE, TIPD_REG_SLEEP_CONFIG, &sleep_config, sizeof(sleep_config), port);
}

/**
 * amd_crb_drivers_tps6699x_deadBattery
 *
 * @param [in]     port;        port for ti pd
 *
...
{
	DRV_TPS6699X_POWER_PATH status;
	...
	/* Read the register */
	ret = amd_crb_drivers_tps6699x_regAccess(TI_REG_READ, TIPD_REG_PWR_PATH_STATUS, &status, sizeof(status), port);
...
/**
 * amd_crb_drivers_tps6699x_portConfigPdDisable
 *
 * @param [in]     port;           port for ti pd
...
{
	DRV_TPS6699X_PORT_CONFIG status, status_set, status_clear;

	for (uint8_t index = 0; index < DRV_TPS6699X_PORT_CONFIG_REGISTER_LENGTH; index++) {
		status_set.buf[index] = 0;
		status_clear.buf[index] = 0;
	}

	amd_crb_drivers_tps6699x_regAccess(TI_REG_READ, TIPD_REG_PORT_CONFIG, &status, sizeof(status), port);
	...
	if ((pd_disable) && (status.f.DisablePD == 0)) {
			...
			amd_crb_drivers_tps6699x_portConfig(port, status_set, status_clear);
	}
...
/**
 * amd_crb_drivers_tps6699x_port_config
 *
 * @param [in]     port;           port for ti pd
...
{
	int ret = 0;
	DRV_TPS6699X_PORT_CONFIG status;
	...
	/* read current port control */
	ret = amd_crb_drivers_tps6699x_regAccess(TI_REG_READ, TIPD_REG_PORT_CONFIG, &status, sizeof(status), port);
	...
	/* write port control back to device */
	amd_crb_drivers_tps6699x_regAccess(TI_REG_WRITE, TIPD_REG_PORT_CONFIG, &status, sizeof(status), port);
}

/**
 * amd_crb_drivers_tps6699x_portConfigStateMachine
 *
 * @param [in]     port;           port for ti pd
...
/**
 * amd_crb_drivers_tps6699x_cmdxRetry
 *
...
uint32_t amd_crb_drivers_tps6699x_cmdxRetry(uint32_t timeout, uint8_t port)
{
	...
	LOG_DBG("retry P:%d %d %d %d %d", port, g_tps6699x_prev_4CC[port][0],
			                                  g_tps6699x_prev_4CC[port][1],
											  g_tps6699x_prev_4CC[port][2],
											  g_tps6699x_prev_4CC[port][3]);

	if (g_tps6699x_prev_len[port] != 0) {
		/* 2. write DataIn to DATA if (dataInLen != 0) */
		memcpy(&tmpBuf[1], g_tps6699x_prev_Data[port], g_tps6699x_prev_len[port]);
		tmpBuf[0] = g_tps6699x_prev_len[port]; /* lenght in first byte */

		amd_crb_drivers_tps6699x_regAccess(TI_REG_WRITE, TIPD_REG_DATA1, tmpBuf, (g_tps6699x_prev_len[port] + 1), port);
	}

	/* 3. write 4-byte pCmd to CMD */
	memcpy(&tmpBuf[1], g_tps6699x_prev_4CC[port], TIPD_REG_CMD1_LEN);  /* both ports have some cmd length */
	tmpBuf[0] = TIPD_REG_CMD1_LEN;
...
/**
 * amd_crb_drivers_tps6699x_portCtrlDrSwapResp
 *
 * @param [in]     port;           port for ti pd
...
{
	DRV_TPS6699X_PORT_CONTROL status_set, status_clear;

	for (uint8_t index = 0; index < DRV_TPS6699X_PORT_CONTROL_REGISTER_LENGTH; index++) {
		status_set.buf[index] = 0;
		status_clear.buf[index] = 0;
	}
	...
	if (resp == APP_RESP_ACCEPT) {
		...
		amd_crb_drivers_tps6699x_portCtrl(port, status_set, status_clear);
	}
...
/**
 * amd_crb_drivers_tps6699x_portCtrlPrSwapResp
 *
 * @param [in]     port;           port for ti pd
...
/**
 * amd_crb_drivers_tps6699x_port_ctl
 *
 * @param [in]     port;           port for ti pd
 *
 * @note
...
{
	int ret = 0;
	DRV_TPS6699X_PORT_CONTROL port_ctrl;
	...
	/* read current port control */
	ret = amd_crb_drivers_tps6699x_regAccess(TI_REG_READ, TIPD_REG_PORT_CTL, &port_ctrl, sizeof(port_ctrl), port);

	if (ret == 0) {
		switch (port_ctrl.f.TypeCCurrent) {
...
void amd_crb_drivers_tps6699x_portCtrl(uint8_t port, DRV_TPS6699X_PORT_CONTROL status_set, DRV_TPS6699X_PORT_CONTROL status_clear)
{
	...
	/* write port control back to device */
	amd_crb_drivers_tps6699x_regAccess(TI_REG_WRITE, TIPD_REG_PORT_CTL, &port_ctrl, sizeof(port_ctrl), port);
}


/**
 * amd_crb_drivers_tps6699x_device_info
 *
 * @param [in]     port;           port for ti pd
 *
 * @note
 *  return the PD device information of image patch full version
 */
uint8_t _amd_crb_drivers_tps6699x_ascii2hex(uint8_t ascii)
{
	if ((ascii >= '0') && (ascii <= '9')) {
		return (ascii - '0');
...
uint32_t amd_crb_drivers_tps6699x_deviceInfo(uint8_t port)
{
	...
	/* Read the register */
	ret = amd_crb_drivers_tps6699x_regAccess(TI_REG_READ, TIPD_REG_DEV_INFO, &status, sizeof(status), port);
	...
	/*TC0: 54 50 53 36 36 39 39 58 20 48 57 30 30 30 30 20 46 57 45 30 31 33 2e 30 38 2e 30 31 5f 30 30 30 36 20 5a 41 63 65 54 00 */
	/*     ( TPS66998 HW0000 FWE013.08.01-0006 ZAceT */

	/*TC1: 54 50 53 36 36 39 39 34 20 4D 31 2E 31 20 47 41 30 30 30 30 30 30 30 20 45 30 31 33 2E 30 39 2E 30 35 5F 30 30 31 44 00 */
	/*     ( TPS66994 M1.1 GA0000000 E013.09.05_001D */

	imageVer = _amd_crb_drivers_tps6699x_ascii2hex(status.buf[38]);
	imageVer += (_amd_crb_drivers_tps6699x_ascii2hex(status.buf[37]) << 4);
	imageVer += (_amd_crb_drivers_tps6699x_ascii2hex(status.buf[36]) << 8);
	imageVer += (_amd_crb_drivers_tps6699x_ascii2hex(status.buf[35]) << 12);
	/* Check device is type */
	return imageVer;
}

/**
 * amd_crb_drivers_tps6699x_autoSink
 *
 * @param [in]     chipID;        PD chip id
 * @param [in]     enable;        true is enable
 *
 * @note
 *  enable the TI pd auto sink control -> select the highest power
 */
void amd_crb_drivers_tps6699x_autoSink(uint8_t chipID, bool enable)
{
	DRV_TPS6699X_GL_SYS_CONFIG status;
	uint32_t ret = 0;

	LOG_DBG("auto sink enable:%d", enable);

	/* Read the register */
	ret = amd_crb_drivers_tps6699x_regAccess(TI_REG_READ, TIPD_REG_GL_SYS_CONFIG, &status, sizeof(status), (chipID << 1));
	status.f.MultiPortSinkPolicy = enable;
	ret = amd_crb_drivers_tps6699x_regAccess(TI_REG_WRITE, TIPD_REG_GL_SYS_CONFIG, &status, sizeof(status), (chipID << 1));
}

/**
 * amd_crb_drivers_tps6699x_lockSinkPath
 *
 * @param [in]     port;          port number
 * @param [in]     enable;        true is locked
 *
 * @note
 *  lock specific PD sink gate
 */
void amd_crb_drivers_tps6699x_lockSinkPath(uint8_t port, bool enable)
{
	DRV_TPS6699X_GL_SYS_CONFIG status;
	uint32_t ret = 0;
	uint8_t config = 0, chipID = 0;
	...
	if (!enable)
		config = 2; /* PB PPEXT is a Sink (input) */
	else
		config = 3; /* PB PPEXT is sink but waits for 'SRDY'. */
	...
	if (port == TYPEC_PORT_0_IDX)
		status.f.PaPpextVbusSwConfig = config;
	else if (port == TYPEC_PORT_1_IDX)
		status.f.PbPpextVbusSwConfig = config;
#if PD_TRIPPORT_ENABLE
	else if (port == TYPEC_PORT_2_IDX)
		status.f.PaPpextVbusSwConfig = config;
#endif
...
/**
 * amd_crb_drivers_tps6699x_portCtrlRetimerUpdate
 *
 * @param [in]     port;          port number
 * @param [in]     enable;        true is force power on
 *
 * @note
...
void amd_crb_drivers_tps6699x_portCtrlRetimerUpdate(uint8_t port, bool enable)
{
	...
	if (enable)
		status_set.f.RetimerFWUpdate = true;
	else
		status_clear.f.RetimerFWUpdate = true;
...
/**
 * amd_crb_drivers_tps6699x_cmdAltMode
 *
 * @param [in]     timeout;        time out for retry
 * @param [in]     port;           port for ti pd
...
void amd_crb_drivers_tps6699x_cmdAltMode(uint32_t timeout, uint8_t port, uint8_t* svid, uint8_t ObjPos, uint8_t * p4CC)
{
	...
	amd_crb_drivers_tps6699x_regAccess(TI_REG_WRITE, TIPD_REG_DATA1, tmpBuf, (TIPD_CMD_ST_ALT_MODE_LEN + 1), port);
	g_tps6699x_prev_len[port] = TIPD_CMD_ST_ALT_MODE_LEN;
	memcpy(&g_tps6699x_prev_Data[port][0], &tmpBuf[1], TIPD_CMD_ST_ALT_MODE_LEN);
...
/**
 * amd_crb_drivers_tps6699x_cmdUcsi
 *
 * @param [in]     timeout;        time out for retry
 * @param [in]     port;           port for ti pd
...
void amd_crb_drivers_tps6699x_cmdUcsi(uint32_t timeout, uint8_t port, uint8_t ucsi_cmd, uint8_t ucsi_length, ucsi_ctrl_details_t *detail)
{
	...
	/* 1. CMD is 0 or '!CMD' */
	//amd_crb_drivers_tps6699x_wait4CmdDone(port, timeout);  /* not used for now */
	...
	if (ucsi_cmd) {
		...
		if (ucsi_cmd != UCSI_CMD_GET_ALTERNATE_MODES) { /* Get alt mode connector num is not in those bits */
			...
			if (port == TYPEC_PORT_2_IDX)
				...
				tmpBuf[4] |= (port + 1);  /* Force add the connector number */
		}
	}

	LOG_DBG("UCSI Start P:%d cmd:%d ucsi_len:%d", port, ucsi_cmd,ucsi_length);

	amd_crb_drivers_tps6699x_regAccess(TI_REG_WRITE, TIPD_REG_DATA1, tmpBuf, (TIPD_CMD_ST_UCSI_LEN + 1), port);
	g_tps6699x_prev_len[port] = tmpBuf[0];
	memcpy(&g_tps6699x_prev_Data[port][0], &tmpBuf[1], TIPD_CMD_ST_UCSI_LEN);
	...
	/* Save the currnet 4CC command */
	memcpy(&g_tps6699x_prev_4CC[port][0], TIPD_CMD_ST_UCSI, TIPD_REG_CMD1_LEN);  /* both ports have some cmd length */
...
/**
 * amd_crb_drivers_tps6699x_portConfigStateMachineMatch
 *
 * @param [in]     port;           port for ti pd
 * @param [in]     state_machine;  typec state machine
 *
...
/**
 * amd_crb_drivers_tps6699x_portCtrlTypecCurrent
 *
 * @param [in]     port;           port for ti pd
...
{
	DRV_TPS6699X_PORT_CONTROL status, status_set, status_clear;
	dpm_status_t *dpm_stat = dpm_set_status(port);
	...
	amd_crb_drivers_tps6699x_regAccess(TI_REG_READ, TIPD_REG_PORT_CTL, &status, sizeof(status), port);
	...
	switch (cc_cur) {
		case DPM_CMD_SET_RP_DFLT:
			...
			amd_crb_drivers_tps6699x_portConfigPdDisable(port, 1); /* disable pd */
		
			if (status.f.TypeCCurrent != TYPEC_CURRENT_DEF) {
				amd_crb_drivers_tps6699x_portConfigStateMachine(port, PRT_NULL);
				amd_crb_drivers_tps6699x_portCtrl(port, status_set, status_clear);
				k_sleep(K_MSEC(4));  /* delay 4ms */
				amd_crb_drivers_tps6699x_portConfigStateMachine(port, PRT_DUAL);
		...
		case DPM_CMD_SET_RP_3A:
			...
			amd_crb_drivers_tps6699x_portConfigPdDisable(port, 0); /* enable pd */
	...
	amd_crb_drivers_tps6699x_readTypecCurrent(port);
}

/**
 * amd_crb_drivers_tps6699x_portCtrlInitDrSwap
 *
 * @param [in]     port;               port for ti pd
 ...
 */
void amd_crb_drivers_tps6699x_portCtrlInitDrSwap(uint8_t port, port_type_t targer_port_type)
{
	DRV_TPS6699X_PORT_CONTROL status_set, status_clear;
...
/**
 * amd_crb_drivers_tps6699x_portCtrlInitPrSwap
 *
 * @param [in]     port;               port for ti pd
 ...
 */
void amd_crb_drivers_tps6699x_portCtrlInitPrSwap(uint8_t port, port_role_t targer_port_role)
{
	DRV_TPS6699X_PORT_CONTROL status_set, status_clear;
...
/**
 * amd_crb_drivers_tps6699x_readCableVdo
 *
 * @param [in]     port;               port for ti pd
 *
 * @return 0 If fail.
...
uint32_t amd_crb_drivers_tps6699x_readCableVdo(uint8_t port)
{
	int ret = 0;
	uint32_t cbl_vdo;
	DRV_TPS6699X_RX_VID_SOPP rx_vid_sopp;
	
	ret = amd_crb_drivers_tps6699x_regAccess(TI_REG_READ, TIPD_REG_RX_IDENTITY_SOPP, &rx_vid_sopp, sizeof(rx_vid_sopp), port);
...
/**
 * amd_crb_drivers_tps6699x_readRxVidSop
 *
 * @param [in]     port;               port for ti pd
 *
 * @note
...
{
	int ret = 0;
	DRV_TPS6699X_RX_VID_SOP rx_vid_sop;
	dpm_status_t *dpm_stat = (dpm_status_t *)dpm_set_status(port);
	pd_do_t ID_Header, Product_VDO, UFP_VDO;
	
	ret = amd_crb_drivers_tps6699x_regAccess(TI_REG_READ, TIPD_REG_RX_IDENTITY_SOP, &rx_vid_sop, sizeof(rx_vid_sop), port);
	...
	if (ret == 0) {
		...
		dpm_stat->partner_vid = ID_Header.std_id_hdr.usb_vid;
		dpm_stat->partner_pid = Product_VDO.std_prod_vdo.usb_pid;
...
/**
 * amd_crb_drivers_tps6699x_readRxSvidSop
 *
 * @param [in]     port;               port for ti pd
 *
 * @note
...
{
	int ret = 0;
	DRV_TPS6699X_RX_SVID_SOP rx_svid_sop;
	dpm_status_t *dpm_stat = (dpm_status_t *)dpm_set_status(port);
	uint32_t svid[2];
	
	ret = amd_crb_drivers_tps6699x_regAccess(TI_REG_READ, TIPD_REG_DIS_SVID, &rx_svid_sop, sizeof(rx_svid_sop), port);
...
/**
 * amd_crb_drivers_tps6699x_readRxVidSopp
 *
 * @param [in]     port;               port for ti pd
 *
 * @note
...
/**
 * amd_crb_drivers_tps6699x_readVdmInfo
 *
 * @param [in]     port;               port for ti pd
 *
 * @note
...
void amd_crb_drivers_tps6699x_readVdmInfo(uint8_t port)
{
	...
	ret = amd_crb_drivers_tps6699x_regAccess(TI_REG_READ, TIPD_REG_RX_VDM, &vdm_info, 6, port);
	...
	if (ret == 0) {
		...
		if(VDM_Header.std_vdm_hdr.cmd == VDM_CMD_DSC_IDENTITY) {
			...
			amd_crb_drivers_tps6699x_readRxVidSop(port);
			amd_crb_drivers_tps6699x_readRxVidSopp(port);
			
		}
		else if (VDM_Header.std_vdm_hdr.cmd == VDM_CMD_DSC_SVIDS) {
			LOG_DBG("Load the svid P:%x", port);
			amd_crb_drivers_tps6699x_readRxSvidSop(port);
		}
...
/**
 * amd_crb_drivers_tps6699x_altModeInfo
 *
 * @param [in]     port;               port for ti pd
 *
 * @note
...
void amd_crb_drivers_tps6699x_altModeInfo(uint8_t port)
{
	...
	/* only add TBT svid 0x8087 and mode 0x0001 into buffer */
	set_alt_mode_info(port, 0, INTEL_VID, 0x0001);
	altmode_num++;
		
	/* only add DP svid 0xFF01 and mode 0x1C46 into buffer */
	set_alt_mode_info(port, 0, DP_SVID, 0x1C46);
	altmode_num++;
...
/**
 * amd_crb_drivers_tps6699x_partnerSrcCapInfo
 *
 * @param [in]     port;               port for ti pd
 *
 * @note
...
{
	dpm_status_t *dpm_stat = (dpm_status_t *)dpm_set_status (port);
	DRV_TPS6699X_RX_SRC_CAP src_cap;
	int ret = 0;
	
	LOG_DBG("Get_partner_SrcCap P:%d", port);
	
	/* Read the register */
	ret = amd_crb_drivers_tps6699x_regAccess(TI_REG_READ, TIPD_REG_RX_SRC_CAP, &src_cap, sizeof(src_cap), port);
...
/**
 * amd_crb_drivers_tps6699x_partnerSnkCapInfo
 *
 * @param [in]     port;               port for ti pd
 *
 * @note
...
{
	dpm_status_t *dpm_stat = (dpm_status_t *)dpm_set_status (port);
	DRV_TPS6699X_RX_SNK_CAP snk_cap;
	uint32_t ret = 0;
	
	LOG_DBG("Get_partner_SnkCap P:%d", port);
	
	/* Read the register */
	ret = amd_crb_drivers_tps6699x_regAccess(TI_REG_READ, TIPD_REG_RX_SINK_CAP, &snk_cap, sizeof(snk_cap), port);
...
/**
 * amd_crb_drivers_tps6699x_devSrcCapInfo
 *
 * @param [in]     port;               port for ti pd
...
{
	dpm_status_t *dpm_stat = (dpm_status_t *)dpm_set_status (port);
	DRV_TPS6699X_TX_SRC_CAP src_cap;
	uint32_t ret = 0;
	uint8_t i;
	...
	/* Read the register */
	ret = amd_crb_drivers_tps6699x_regAccess(TI_REG_READ, TIPD_REG_TX_SRC_CAP, &src_cap, sizeof(src_cap), port);
...
/**
 * amd_crb_drivers_tps6699x_refreashDevSrcCapCurrent
 *
 * @param [in]     port;               port for ti pd
 *
 * @note
...
/**
 * amd_crb_drivers_tps6699x_devSrcCapUpdate
 *
 * @param [in]     port;        port for ti pd
...
void amd_crb_drivers_tps6699x_devSrcCapUpdate(uint8_t port, uint8_t mask, pd_do_t* pdos)
{
	...
	if (ret == 0) {
		...
		ret = amd_crb_drivers_tps6699x_regAccess(TI_REG_WRITE, TIPD_REG_TX_SRC_CAP, &src_cap, sizeof(src_cap), port);
...
/**
 * amd_crb_drivers_tps6699x_devSrcCapDeft
 *
 * @param [in]     port;        port for ti pd
 *
 * @note
...
void amd_crb_drivers_tps6699x_devSrcCapDeft(uint8_t port)
{
	...
	if (ret == 0) {
		...
		amd_crb_drivers_tps6699x_refreashDevSrcCapCurrent(port);
	}
}

/**
 * amd_crb_drivers_tps6699x_devSnkCapInfo
 *
 * @param [in]     port;        port for ti pd
...
{
	dpm_status_t *dpm_stat = (dpm_status_t *)dpm_set_status (port);
	DRV_TPS6699X_TX_SNK_CAP snk_cap;
	int ret = 0;
	uint8_t i;
	...
	/* Read the register */
	ret = amd_crb_drivers_tps6699x_regAccess(TI_REG_READ, TIPD_REG_TX_SNK_CAP, &snk_cap, sizeof(snk_cap), port);
	...
	amd_crb_drivers_tps6699x_regAccess(TI_REG_WRITE, TIPD_REG_TX_SNK_CAP, &snk_cap, sizeof(snk_cap), port);
...
/**
 * amd_crb_drivers_tps6699x_refreashDevSnkCapCurrent
 *
 * @param [in]     port;        port for ti pd
 *
 * @note
...
/**
 * amd_crb_drivers_tps6699x_devSnkCapUpdate
 *
 * @param [in]     port;        port for ti pd
...
void amd_crb_drivers_tps6699x_devSnkCapUpdate(uint8_t port, uint8_t mask, pd_do_t* pdos)
{
	...
	if (ret == 0) {
		...
		/* update the src cap back  */
		ret = amd_crb_drivers_tps6699x_regAccess(TI_REG_WRITE, TIPD_REG_TX_SNK_CAP, &snk_cap, sizeof(snk_cap), port);
...
/**
 * amd_crb_drivers_tps6699x_devSnkCapDeft
 *
 * @param [in]     port;        port for ti pd
 *
 * @note
...
void amd_crb_drivers_tps6699x_devSnkCapDeft(uint8_t port)
{
	...
	if (ret == 0) {
		...
		amd_crb_drivers_tps6699x_refreashDevSnkCapCurrent(port);
	}
}
